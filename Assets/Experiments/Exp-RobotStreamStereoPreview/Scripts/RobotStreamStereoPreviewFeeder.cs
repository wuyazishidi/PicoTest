// Assets/Experiments/Exp-RobotStreamStereoPreview/Scripts/RobotStreamStereoPreviewFeeder.cs
using System;
using System.Collections;
using System.IO;
using System.Reflection;
using PicoTest.Experiments.RobotDsDome;
using PicoTest.Experiments.RobotStreamLeftPreview;
using PicoTest.Experiments.WebRTC;
using PicoTest.Rendering;
using UnityEngine;
using UnityEngine.XR;

namespace PicoTest.Experiments.RobotStreamStereoPreview
{
    /// <summary>
    /// Robot Stream Stereo Preview Demo：直连 Tools/run_stereo_viewer.py 的 aiortc HTTP
    /// offer/answer 端点（转发完整 SBS 双目画面，不裁剪），用 VstPassthroughDemo 同款穹顶显示
    /// 方案呈现——左右眼真正分半显示（同 Exp-RobotStream 的 SBS 用法）。
    /// 最大化复用 Exp-RobotStreamLeftPreview：直接引用它的 HttpOfferVideoSource（同一套 HTTP
    /// offer/answer 握手，双目/单目对它没有区别，只是喂的画面宽了一倍）和 Resources/SwapRB.shader
    /// （Resources.Load 按虚拟路径合并全项目 Resources/ 目录，无需复制）。标定用 Exp-RobotDsDome
    /// 的 Double Sphere 模型（机器人真实相机模型），左右眼各用各的真实标定。
    /// 另起炉灶：不改 Exp-WebRTC / Exp-RobotStream / Exp-RobotStreamLeftPreview / Exp-RobotDsDome
    /// / Exp-VstPassthrough / Main，仅引用复用。
    /// </summary>
    public sealed class RobotStreamStereoPreviewFeeder : MonoBehaviour
    {
        [Header("DS 标定（左右各一；跑 PicoTest/Robot DS Dome/Import Camchain 生成）")]
        public DsEyeCalibration leftCalibration, rightCalibration;
        [Header("视频源")]
        public bool useRealWebRtc = true;                                // false=假帧源（编辑器冒烟）
        public string serverUrl = "http://127.0.0.1:8889";               // run_stereo_viewer.py 地址（双目，8889）
        [Header("穹顶（DS 是宽鱼眼，coverageDeg 起点比等距鱼眼大；真机 cmd 调）")]
        public float coverageDeg = 190f;
        public float radius = 2.0f;
        public float edgeFeatherDeg = 8f;
        [Header("画面朝向（真机验证：com.unity.webrtc 解码纹理是倒的→flipV=1，2026-07-17 实测）")]
        [Range(0, 1)] public float flipV = 1f;
        [Range(0, 1)] public float mirror = 0f;
        [Header("颜色修正（源端 R/B 天生互换，见 run_stereo_viewer.py 文档；默认开）")]
        ...
        [Header("低速云台伺服（参照 Exp-VstPassthrough 的 VstCameraDomeFeeder：混合转向慢分量，" +
                 "转头超死区才低速把穹顶带回居中；机器人真实位姿暂缺时的舒适度替代方案）")]
        ...
        // 颜色修正：把源纹理 Blit 到本地 RT（R/B 互换），穹顶采样这张 RT（左右眼各取一半 UV）
        ...
        private void Start()
        {
            ...
            {
                Debug.LogError("[RobotStereo] 未连 DS 标定（leftCalibration/rightCalibration）。先跑 PicoTest/Robot DS Dome/Import Camchain。");
                return;
            }
            if (enableSeeThrough) EnableVideoSeeThrough();

            _anchor = new GameObject("RobotStereoDomeAnchor").transform;
            ...
            // 低速云台伺服：转头超死区才把穹顶锚点偏航低速带回居中（保居中、可看 FOV 外），
            // 同 VstCameraDomeFeeder 的用法——机器人真实位姿暂缺，先用头显自身朝向当目标角。
            ...
            else Debug.LogWarning("[RobotStereo] 未找到 SwapRB shader（Resources/SwapRB.shader），颜色修正已禁用。");

            CreateHud();

            ResolveServerUrl();   // 运行时覆盖：files/robotstereopreview/server.txt 优先于场景默认（免重打包换 IP）
            StartSource(Source);

            Debug.Log($"[RobotStereo] 启动：src={(useRealWebRtc ? "webrtc" : "fake")} server={serverUrl} mode={poseMode} " +
                      $"radius={radius} latency={latencyMs}ms swapRB={swapRB}");
        }

        /// <summary>启动/切换视频源；WebRTC 渲染泵（全局 WebRTC.Update()）只驱动一次。
        /// 复用 Exp-RobotStreamLeftPreview 的 HttpOfferVideoSource——同一套 HTTP offer/answer
        /// 握手，双目/单目对它没有区别，只是画面宽了一倍。</summary>
        ...
        /// <summary>
        /// 服务地址运行时覆盖：读 persistentDataPath/robotstereopreview/server.txt（一行
        ...
        private void ResolveServerUrl()
        {
            try
            {
                string path = Path.Combine(Application.persistentDataPath, "robotstereopreview", "server.txt");
                if (!File.Exists(path)) return;
                string u = File.ReadAllText(path).Trim();
                if (!string.IsNullOrEmpty(u)) { serverUrl = u; Debug.Log($"[RobotStereo] 服务地址覆盖自 server.txt → {u}"); }
            }
            catch (Exception e) { Debug.LogWarning($"[RobotStereo] 读 server.txt 失败: {e.Message}"); }
        }
        ...
        private void ReconnectServer(string url)
        {
            ...
            Source = null;
            StartSource(null);
            Debug.Log($"[RobotStereo] 重连服务 → {url}");
        }
        ...
        /// <summary>
        /// 穹顶锚点姿态：
        /// - WorldLocked：参照 FisheyeDomeXRRig（Main 里的正式实现）——只让位置跟头显走（始终
        ///   罩住眼点），朝向完全不碰，转头在静止穹顶内环顾。机器人真实位姿暂时传不过来，先用
        ///   这个跟主线一致的默认档位。
        ...
        // ── adb 调参通道（照搬 RobotStreamLeftPreview cmd.txt 模式）─────────────────
        // adb shell "echo mode captureproxy > /sdcard/Android/data/<pkg>/files/robotstereopreview/cmd.txt"
        ...
        private void PollCmdFile()
        {
            ...
            try
            {
                string path = Path.Combine(Application.persistentDataPath, "robotstereopreview", "cmd.txt");
                if (!File.Exists(path)) return;
                string raw = File.ReadAllText(path).Trim();
                File.Delete(path);
                if (raw.Length == 0) return;
                Debug.Log($"[RobotStereo] cmd.txt → \"{raw}\"");
                foreach (var line in raw.Split('\n')) ExecCmd(line.Trim());
            }
            catch (Exception e) { Debug.LogWarning($"[RobotStereo] cmd.txt 处理失败: {e.Message}"); }
        }
        ...
        private void ExecCmd(string cmd)
        {
            ...
            switch (parts[0].ToLowerInvariant())
            {
                ...
                case "colorfix":
                    swapRB = arg != "off";
                    Debug.Log($"[RobotStereo] 颜色修正 → {(swapRB ? "on" : "off")}");
                    break;
                case "cover" when TryF(arg, out float cov):
                    ...
                    Debug.Log("[RobotStereo] cover 只更新羽化阈值（裁剪），网格弧度下次启动生效");
                    break;
                ...
                case "servo":
                    enableGazeServo = arg != "off";
                    if (_servo != null) _servo.followLocalHead = enableGazeServo;
                    Debug.Log($"[RobotStereo] 云台伺服 → {(enableGazeServo ? "on" : "off")}");
                    break;
                case "servorate" when TryF(arg, out float sr):
                    servoRateDegPerSec = Mathf.Clamp(sr, 1f, 360f);
                    if (_servo != null) _servo.rateDegPerSec = servoRateDegPerSec;
                    break;
                case "servodead" when TryF(arg, out float sd):
                    servoDeadzoneDeg = Mathf.Clamp(sd, 0f, 179f);
                    if (_servo != null) _servo.deadzoneDeg = servoDeadzoneDeg;
                    break;
                case "servoreturn" when TryF(arg, out float srt):
                    servoReturnDeg = Mathf.Clamp(srt, 0f, 179f);
                    if (_servo != null) _servo.returnDeg = servoReturnDeg;
                    break;
                case "dump":
                    Debug.Log($"[RobotStereo] dump: src={(useRealWebRtc ? "webrtc" : "fake")} server={serverUrl} mode={poseMode} " +
                              ...
                              $"servo={enableGazeServo} servoRate={servoRateDegPerSec} servoDead={servoDeadzoneDeg} servoReturn={servoReturnDeg}");
                    break;
                default:
                    Debug.LogWarning($"[RobotStereo] 未知命令：{cmd}"); break;
            }
        }
        ...
        private void SetDomeVisible(bool on)
        {
            ...
            Debug.Log($"[RobotStereo] 穹顶 {(on ? "显示" : "隐藏 → 原生透视对比")}");
        }
        ...
        private void UpdateHud()
        {
            ...
            _hud.text = $"RobotStereoPreview (DS)  src={(useRealWebRtc ? "webrtc" : "fake")} {_measuredFps:F0}fps frame={(_lastFrame != null ? "yes" : "--")}\n" +
                        ...
                        $"A=对比原生透视  B=退出  cmd: files/robotstereopreview/cmd.txt";
        }
        ...
        private static void EnableVideoSeeThrough()
        {
            ...
            foreach (var tn in typeNames)
            {
                try
                {
                    ...
                    if (prop != null && prop.CanWrite) { prop.SetValue(null, true); any = true; Debug.Log($"[RobotStereo] 透视已开启 via {tn}"); continue; }
                    var field = ty.GetField("EnableVideoSeeThrough", BindingFlags.Public | BindingFlags.Static);
                    if (field != null) { field.SetValue(null, true); any = true; Debug.Log($"[RobotStereo] 透视已开启 via {tn}"); }
                }
                catch (Exception e) { Debug.LogWarning($"[RobotStereo] 设 {tn} 透视失败: {e.Message}"); }
            }
            if (!any) Debug.LogWarning("[RobotStereo] 未找到透视 API，透视未开启");
        }
        ...
        private void StartQuit()
        {
            if (_quitting) return;
            _quitting = true;
            Debug.Log($"[RobotStereo] B 键退出：停源，{quitDelaySec:F0}s 后退出程序");
        ...
        private IEnumerator QuitAfter()
        {
            ...
            Debug.Log("[RobotStereo] 退出程序（killProcess）");
            ...
            catch (Exception e) { Debug.LogWarning($"[RobotStereo] killProcess 失败: {e.Message}"); }
#elif UNITY_EDITOR
